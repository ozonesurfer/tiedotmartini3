...
func GetDB() *db.DB {
	myDB, err := db.OpenDB(tiedotmartini3.DATABASE_DIR)
	if err != nil {
		panic(err)
	}
	return myDB
...
func (this *Album) GetGenreName() string {
	...
	rawDoc, _ := GetDoc(id, tiedotmartini3.GENRE_COL)
...
func GetGenreName(id string) string {
	id2 := ToObjectId(id)
	rawDoc, _ := GetDoc(id2, tiedotmartini3.GENRE_COL)
...
func GetBandsByGenre(id string) []DocWithID {
	database := GetDB()
	defer database.Close()
	collection := database.Use(tiedotmartini3.BAND_COL)
	var query interface{}
...
func (this DocWithID) GetLocation() string {
	...
	rawDoc, err := GetDoc(id, tiedotmartini3.LOCATION_COL)
	if err != nil {
		return err.Error()
	}
